<script>
  ...
  let offsetDistanceMiles = $derived(Math.round(Math.abs(fuzz) * 69));
  let offsetDistanceKm = $derived(Math.round(Math.abs(fuzz) * 111));
  ...
  <div class="sections-grid">
      ...
      >
        ...
        <div class="form-group">
          ...
          {#if isActive}
            ...
            <p class="field-hint">
              Warning: Setting to Dormant will close your club permanently.
            </p>
          {:else}
            <input
              type="text"
            ...
            <p class="field-hint">
              Club status cannot be changed once dormant.
            </p>
          {/if}
        </div>
        ...
        <div class="form-group">
          ...
          />
          <p class="field-hint">
            Optional. Your club's website or landing page URL.
          </p>
          {#if clubWebsite}
            <p class="redirect-link">
              <span class="redirect-label">Club redirect:</span>
              <a
                href="https://hack.club/club/{getClubSlug(club.name)}"
                target="_blank"
    ...
    <section class="settings-section">
      ...
      <p class="section-intro">
        Control how your club appears on the <a
          href="https://hackclub.github.io/map"
          target="_blank"
          rel="noopener noreferrer">Hack Club Map</a
        >.
      </p>

      {#if form?.mapSuccess}
        <div class="success-message">
          {form.message || "Map settings updated!"}
        </div>
      {/if}
      ...
      <form
        method="POST"
        action="?/updateMapSettings"
        onsubmit={() => (isMapSaving = true)}
      >
        ...
        <div class="form-group">
          ...
          />
          <p class="field-hint">
            Find your coordinates on <a
              href="https://www.latlong.net/"
              target="_blank"
              rel="noopener noreferrer">latlong.net</a
            >
          </p>
        </div>
        ...
        <div class="form-group">
          ...
          <input
            ...
            step="0.01"
            disabled={optedOut}
            style="padding: 0; height: 8px; background: #e0e6ed; appearance: none; -webkit-appearance: none; border-radius: 4px;"
          />
          {#if fuzz === 0 || fuzz === 0.0}
            <p class="field-hint" style="color: #3b82f6;">
              Your exact location will be shown on the public map.
            </p>
          {:else}
            <p class="field-hint" style="color: #3b82f6;">
              Your club pin will be randomized by approximately <strong
                >{offsetDistanceMiles} miles</strong
              >
              ({offsetDistanceKm} km) for privacy.
            </p>
          {/if}
          <p class="field-hint">
            Add a small offset to your exact location for privacy.
          </p>
        </div>
    ...
    <section class="settings-section">
      ...
      <a href="/invite-coleader" class="action-btn"> + Add Co-Leader </a>
    </section>
  </div>
</div>
  ...
  <div class="modal-overlay" role="dialog" aria-modal="true">
    <div class="modal">
      ...
      <div class="modal-body">
        <p>
          Are you sure you want to set your club to <strong>Dormant</strong>?
        </p>
        <p class="warning-text">
          This action will close your club and <strong>cannot be undone</strong
          >. You will not be able to reactivate the club.
        </p>
      </div>
      <div class="modal-footer">
        <button type="button" class="cancel-btn" onclick={cancelDormant}
          >Cancel</button
        >
        <button type="button" class="danger-btn" onclick={confirmDormant}
          >Yes, Make Dormant</button
        >
      </div>
    </div>
  </div>
